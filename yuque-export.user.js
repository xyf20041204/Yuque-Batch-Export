...
// @version      2.1.2
// @description  批量导出语雀知识库文档，支持 Markdown / Lake / PDF / Word / JPG 格式
...
(function () {
  ...
  async function fetchTocData(username, bookSlug, cookieHeader) {
    const response = await fetch('https://www.yuque.com/' + username + '/' + bookSlug, { headers: { 'cookie': cookieHeader, 'accept': 'text/html' } });
  ...
  async function fetchBookOwnerMap(cookieHeader) {
    const response = await fetch('https://www.yuque.com/api/mine/book_stacks', { headers: { 'accept': 'application/json', 'cookie': cookieHeader } });
  ...
  async function downloadSingleDoc(doc, ctx) {
    const { currentBook, docPathMap, username, options, cookieHeader, progress: progressCtx } = ctx;
    ...
    try {
      ...
      for (let retry = 0; retry < 3; retry++) {
        const r = await fetch('https://www.yuque.com/api/docs/' + doc.id + '/export', {
          method: 'POST',
          headers: { 'content-type': 'application/json', 'cookie': cookieHeader, 'referer': 'https://www.yuque.com/' + username + '/' + currentBook.slug },
          body: JSON.stringify(exportPayload),
        });
        if (!r.ok) throw new Error('Export request failed: HTTP ' + r.status);
        const d = await r.json();
        if (d.data?.state === 'success') { exportResult = d.data; break; }
        await sleep(3000);
      }
      if (!exportResult?.url) throw new Error('Export timeout or failed');
      let dUrl = exportResult.url;
      if (dUrl.startsWith('/')) dUrl = 'https://www.yuque.com' + dUrl;
      const fileResponse = await fetch(dUrl, { headers: { 'cookie': cookieHeader } });
  ...
  async function handleBatchExport(bookList, options) {
    try {
      ...
      if (!cookieHeader) throw new Error('无法获取 Cookie');
      ...
      for (let bi = 0; bi < bookList.length; bi++) {
        ...
        const docsResponse = await fetch('https://www.yuque.com/api/docs?book_id=' + currentBook.id, { headers: { 'accept': 'application/json', 'cookie': cookieHeader } });
        ...
        const results = await mapWithConcurrency(docs, concurrency, (doc) => downloadSingleDoc(doc, { currentBook, docPathMap, username, options, cookieHeader, progress: progressCtx }), 1000);
        const successCount = results.filter(Boolean).length;
        totalSuccess += successCount;
        sendProgress({ bookName: currentBook.name, current: docs.length, total: docs.length, bookIndex: bi + 1, bookTotal: bookList.length, filename: currentBook.name + ' 完成', bookDone: true, successCount: successCount });
      }
      sendProgress({ current: totalCount, total: totalCount, filename: '全部完成', done: true, successCount: totalSuccess });
  ...
  panel.innerHTML = '<div id="yuque-export-icon"><img src="' + iconUrl + '" alt="logo"></div><div id="yuque-export-header"><div id="yuque-export-header-title"><img src="' + iconUrl + '" alt="logo"><h3>语雀批量导出插件</h3></div><div><button id="ye-collapse-btn" title="折叠">−</button></div></div><div id="yuque-export-body"><div class="ye-section"><label class="ye-label">选择知识库</label><button id="ye-refresh-btn" class="ye-btn ye-btn-secondary" style="margin-bottom:8px;">&#128260; 加载知识库</button><label class="ye-select-all" style="display:none;"><input type="checkbox" id="ye-select-all"> 全选</label><div id="ye-book-list" class="ye-book-list"><div style="padding:20px;text-align:center;color:#9ca3af;font-size:12px;">点击上方按钮加载知识库</div></div><div id="ye-selected-hint" class="ye-hint">未选择知识库</div></div><div class="ye-section"><label class="ye-label">导出格式</label><div class="ye-options"><label class="ye-option"><input type="radio" name="ye-format" value="markdown" checked> Markdown</label><label class="ye-option"><input type="radio" name="ye-format" value="lake"> Lake</label><label class="ye-option"><input type="radio" name="ye-format" value="pdf"> PDF</label><label class="ye-option"><input type="radio" name="ye-format" value="word"> Word</label><label class="ye-option"><input type="radio" name="ye-format" value="jpg"> JPG</label></div></div><div class="ye-section" id="ye-pdf-options" style="display:none;"><label class="ye-label">PDF 选项</label><div class="ye-options"><label class="ye-option"><input type="checkbox" id="ye-opt-toc" checked> 导出大纲</label></div></div><div class="ye-section" id="ye-md-options"><label class="ye-label">导出选项</label><div class="ye-options"><label class="ye-option"><input type="checkbox" id="ye-opt-latexcode"> LaTeX 公式</label><label class="ye-option"><input type="checkbox" id="ye-opt-anchor"> 锚点</label><label class="ye-option"><input type="checkbox" id="ye-opt-linebreak"> 换行</label><label class="ye-option"><input type="checkbox" id="ye-opt-usemdai" checked> MDAI</label></div></div><div class="ye-section"><label class="ye-label">下载并发数</label><div class="ye-rate-limit"><select id="ye-rate-limit"><option value="1">1</option><option value="2" selected>2</option><option value="4">4</option><option value="8">8</option></select></div></div><button id="ye-export-btn" class="ye-btn" disabled>&#128229; 批量导出</button><div id="ye-progress" class="ye-progress"><div class="ye-progress-track"><div id="ye-progress-fill" class="ye-progress-fill"></div></div><div id="ye-progress-text" class="ye-progress-text">准备中...</div></div><div id="ye-status" class="ye-status"></div></div>';
  ...
  function updateSelectedBooks() { const all = bookListEl.querySelectorAll('.ye-book-checkbox'); const checked = bookListEl.querySelectorAll('.ye-book-checkbox:checked'); selectAllCheckbox.checked = all.length > 0 && checked.length === all.length; const ids = []; checked.forEach(cb => ids.push(parseInt(cb.dataset.bookId))); selectedBooks = books.filter(b => ids.includes(b.id)); if (selectedBooks.length === 0) { selectedHint.textContent = '未选择知识库'; selectedHint.style.color = '#9ca3af'; exportBtn.disabled = true; } else { const total = selectedBooks.reduce((s, b) => s + b.items_count, 0); selectedHint.textContent = '已选择 ' + selectedBooks.length + ' 个知识库 (' + total + ' 篇)'; selectedHint.style.color = '#374151'; exportBtn.disabled = false; } }
  refreshBtn.addEventListener('click', async () => { try { refreshBtn.disabled = true; refreshBtn.textContent = '加载中...'; books = await getBookStacks(); renderBookList(books); refreshBtn.disabled = false; refreshBtn.innerHTML = '&#128260; 加载知识库'; } catch (err) { bookListEl.innerHTML = '<div style="padding:20px;text-align:center;color:#dc2626;font-size:12px;">加载失败: ' + err.message + '</div>'; refreshBtn.disabled = false; refreshBtn.innerHTML = '&#128260; 加载知识库'; } });
  exportBtn.addEventListener('click', async () => { if (selectedBooks.length === 0) { showStatus('请先选择知识库', 'error'); return; } const format = document.querySelector('input[name="ye-format"]:checked').value; const options = { format, attachment: true, latexcode: format !== 'lake' && document.getElementById('ye-opt-latexcode').checked, anchor: format !== 'lake' && document.getElementById('ye-opt-anchor').checked, linebreak: format !== 'lake' && document.getElementById('ye-opt-linebreak').checked, useMdai: format !== 'lake' && document.getElementById('ye-opt-usemdai').checked, toc: format === 'pdf' && document.getElementById('ye-opt-toc').checked, concurrency: parseInt(document.getElementById('ye-rate-limit').value, 10) || 2 }; exportBtn.disabled = true; refreshBtn.disabled = true; progressBar.classList.add('active'); progressText.textContent = '正在启动下载任务...'; showStatus('下载任务已启动', 'info'); handleBatchExport(selectedBooks, options).then(result => { exportBtn.disabled = false; refreshBtn.disabled = false; if (result.success) { showStatus('批量导出完成！已下载 ' + result.count + ' 个文件', 'success'); } else { showStatus('批量导出失败: ' + (result.error || '未知错误'), 'error'); } setTimeout(() => { progressBar.classList.remove('active'); progressFill.style.width = '0%'; }, 3000); }); });
  progressListeners.push(msg => { if (msg.done) { exportBtn.disabled = false; refreshBtn.disabled = false; if (msg.error) { showStatus('下载失败: ' + msg.error, 'error'); } else if (msg.successCount !== undefined) { showStatus('批量导出完成！已下载 ' + msg.successCount + ' 个文件', 'success'); } setTimeout(() => { progressBar.classList.remove('active'); progressFill.style.width = '0%'; }, 3000); } else { progressFill.style.width = ((msg.current / msg.total) * 100) + '%'; const prefix = msg.bookTotal > 1 ? '[' + msg.bookIndex + '/' + msg.bookTotal + ' ' + msg.bookName + '] ' : ''; progressText.textContent = prefix + msg.current + '/' + msg.total + ' - ' + (msg.filename || '处理中...'); } });
})();
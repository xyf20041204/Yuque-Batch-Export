...
async function handleExportAndDownload(req) {
  ...
  // 2. Fetch the exported file (auto-sends all cookies)
  ...
  // 3. Convert blob to data URL and send back to background for proper download
  //    (background has chrome.downloads which supports subdirectory paths)
  ...
  console.log('[cs] exportAndDownload DONE, dataUrl length:', dataUrl.length);
  return { success: true, dataUrl: dataUrl, filename: filePath };
}
